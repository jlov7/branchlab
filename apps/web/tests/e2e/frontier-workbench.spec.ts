...
test("frontier workbench: causality, evals, runtime, evidence", async ({ page }) => {
  ...
  await page.getByLabel("Investigation title").fill("Pricing evidence hypothesis");
  await page.getByLabel("Hypothesis").fill("The divergent tool result explains the final quote.");
  ...
  await expect(page.getByText("Pricing evidence hypothesis")).toBeVisible();
